...
impl BackendClient {
    ...
    /// Fee-aware flip candidates ranked from the whole cached market (backend-computed).
    pub async fn get_flips(&self, limit: u32) -> Result<Vec<Flip>, ()> {
        let resp: FlipsResponse = self.get_json(&format!("apps/tbwatcher/flips?limit={limit}")).await?;
        Ok(resp.flips)
    }
...
#[derive(Deserialize)]
struct FlipsResponse {
    flips: Vec<Flip>,
}

/// One ranked flip candidate (backend-computed over the whole cached market).
