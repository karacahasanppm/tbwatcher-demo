...
/// Fee-aware flip candidates from the backend cache (backend-ranked over the whole market). Empty when the
/// backend is off or unreachable — the Flips view just shows nothing then.
#[tauri::command]
async fn flips_get(state: State<'_, AppState>) -> Result<Vec<backend::Flip>, String> {
    match state.backend.clone() {
        Some(client) => Ok(client.get_flips(50).await.unwrap_or_default()),
        None => Ok(Vec::new()),
    }
}
